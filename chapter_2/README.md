...
| `--local-ip 192.168.100.1` | DPDK 服务端 IP |
...
ping -c 2 -I dpdk_tap 192.168.100.1

# 验证 UDP Echo
echo "HELLO DPDK" | nc -u -s 192.168.100.2 -p 9999 192.168.100.1 8888
```
...
def main():
    ...
    parser.add_argument('--ip', default='192.168.100.1')
    parser.add_argument('--port', type=int, default=8888)
    parser.add_argument('--size', type=int, default=64)
    parser.add_argument('--count', type=int, default=50000)
    parser.add_argument('--local-ip', default='192.168.100.2')
...
│  (内核协议栈)  │ ──────→ │  (用户态协议栈)           │
│              │ ←────── │                          │
│ 192.168.100.2 │  Echo   │  192.168.100.1:8888      │
│              │         │  lcore 0: RX/TX 主循环     │
...
### 备选模式：AF_PACKET（绑定物理网卡）

如果有真实网卡（如 `enp2s0`），可替换步骤 3 的 `--vdev` 参数：

```bash
sudo ./build/dpdk_udp \
    -l 0-2 -n 4 -m 256 \
    --vdev=eth_af_packet0,iface=enp2s0 \
    -- \
    --local-ip 192.168.100.1 \
    --local-port 8888
```
