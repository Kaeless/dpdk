...
static uint16_t rte_ipv4_icmp_checksum(uint16_t* addr,int count){
    register long sum = 0;
    while(count > 1){
...
static void ng_encode_icmp_pkt(uint8_t *pkt_data,uint8_t* src_mac,uint8_t* dst_mac,uint32_t sip,
        uint32_t dip,rte_be16_t icmp_ident,rte_be16_t icmp_seq_nb){
    //encode
    ...
    //3.icmp
    struct rte_icmp_hdr *icmp = (struct rte_icmp_hdr *)(pkt_data + 
        sizeof(struct rte_ether_hdr)+ sizeof(struct rte_ipv4_hdr));
    icmp->icmp_type = RTE_IP_ICMP_ECHO_REPLY;
    icmp->icmp_code = 0;
    icmp->icmp_ident = icmp_ident;
    icmp->icmp_seq_nb = icmp_seq_nb;

    icmp->icmp_cksum = 0;
    icmp->icmp_cksum = rte_ipv4_icmp_checksum((uint16_t*)icmp,sizeof(struct rte_icmp_hdr));
...
static struct rte_mbuf* ng_send_icmp(struct rte_mempool *mbuf_pool,uint8_t* src_mac,uint8_t* dst_mac,
	...
	ng_encode_icmp_pkt(pkt_data,src_mac, dst_mac, sip, dip,icmp_ident,icmp_seq_nb);
	...
	while (1) {
		...
		for (i = 0;i < num_recvd;i ++) {
			...
			if(iphdr->next_proto_id == IPPROTO_ICMP){
                ...
                if(icmp_hdr->icmp_type == RTE_IP_ICMP_ECHO_REQUEST){ // requeset icmp protocol


                    struct rte_mbuf *txbuf = ng_send_icmp(mbuf_pool,ehdr->d_addr.addr_bytes,ehdr->s_addr.addr_bytes,
